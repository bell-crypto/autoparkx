<?php
// api/cron_booking_soon.php
// แจ้งเตือนล่วงหน้าก่อนถึงเวลาเริ่มจองประมาณ 10–15 นาที ผ่าน LINE Messaging API (Flex)
...
function line_push_raw(array $body): array {
    ...
    if (!$ok) {
        error_log("LINE booking_soon push fail http={$http} err={$errNo} {$errTx} resp={$resp}");
    }
...
/**
 * สร้าง Flex แจ้งเตือนก่อนถึงเวลา
 */
function build_booking_soon_flex(
    int $bookingId,
    string $slotLabel,
    string $startLabel,
    string $endLabel,
    ...
    return [
        'type' => 'flex',
        'altText' => "AutoParkX: ใกล้ถึงเวลาเริ่มจอง #{$bookingId}",
        'contents' => [
            'type' => 'bubble',
            'size' => 'mega',
            'header' => [
                ...
                'contents' => [
                    ...
                    [
                        ...
                        'contents' => [
                            ...
                            flex_kv_row('สถานะ :', 'ใกล้ถึงเวลาเริ่มจอง'),
                        ],
                    ],
                    [
                        'type' => 'separator',
                    ...
                    [
                        'type' => 'text',
                        'text' => 'กรุณาไปยังจุดจอดให้ตรงเวลา',
                        'size' => 'xs',
                        'color' => '#374151',
                        'wrap' => true,
                        'margin' => 'md',
            ...
            'footer' => [
                ...
                'contents' => [
                    ...
                    [
                        'type' => 'text',
                        'text' => 'AutoParkX • Booking Reminder',
                        'size' => 'xxs',
                        'color' => '#6b7280',
                        'align' => 'center',
                        'margin' => 'sm',
...
try {
    $sql = "
        ...
        WHERE b.booking_soon_sent = 0
          AND b.start_time BETWEEN DATE_ADD(NOW(), INTERVAL 10 MINUTE)
    ...
    foreach ($rows as $r) {
        ...
        $bookingSoon = array_key_exists('booking_soon', $r) && $r['booking_soon'] !== null ? (int)$r['booking_soon'] : 1;
        ...
        if (!$enabled || !$bookingSoon) {
            error_log("[AutoParkX] booking_soon skipped booking_id={$bookingId} prefs_disabled");
            $skipCount++;
            continue;
        }

        if (!looks_like_line_user_id($lineUserId)) {
            error_log("[AutoParkX] booking_soon skipped booking_id={$bookingId} invalid_line_user_id");
            $skipCount++;
            continue;
        }
        ...
        $minsLeft = (int)floor(($start->getTimestamp() - $now->getTimestamp()) / 60);
        ...
        if ($send['ok']) {
            $up = $pdo->prepare("
                UPDATE bookings
                SET booking_soon_sent = 1,
                    booking_soon_sent_at = NOW(),
                    updated_at = NOW()
                WHERE id = :id
                LIMIT 1
            ");
            ...
            $sentCount++;
            error_log("[AutoParkX] booking_soon sent booking_id={$bookingId} user_id={$userId}");
        } else {
            error_log("[AutoParkX] booking_soon send_failed booking_id={$bookingId} http={$send['http']}");
        }
    }
    ...
    error_log('[AutoParkX] cron_booking_soon error: ' . $e->getMessage());
